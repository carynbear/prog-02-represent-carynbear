 ...
 */
public class RepListAdapter extends ArrayAdapter<RepListItem> {
    RepListFragment fragment;

    public RepListAdapter(Context context, List<RepListItem> items, RepListFragment fragment){
        super(context, R.layout.replist_item, items);
...
//            });
        ...
        RepListItem item = getItem(position);
        viewHolder.ivPic.setImageDrawable(item.pic);
        viewHolder.ivPic2.setImageDrawable(item.pic);
        viewHolder.tvParty.setText(item.party);
        viewHolder.tvName.setText(item.name);
        viewHolder.tvTitle.setText(item.title);
        viewHolder.tvEmail.setText(item.email);
        viewHolder.tvWeb.setText(item.web);
        viewHolder.tvTweet.setText(item.tweet);
